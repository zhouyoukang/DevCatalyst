...
### 命令执行优化
- 只读命令→SafeToAutoRun=true | 服务器/watch→Blocking=false
- 禁止造成Ctrl+C场景：持续运行的命令必须用非阻塞模式

## Hooks 策略
- Python/Node.js安全可用；**PowerShell绝对禁止**（干扰提示符检测→全窗口卡死）
- 三层合并：system→user→workspace | Pre-hooks exit code 2 阻止操作
...
## Windsurf 高级功能
- **Codemaps**：代码结构图 | **Worktrees**：多Cascade隔离 | **Vibe&Replace**：AI查找替换
- **Knowledge Base**：自定义知识库 | **Linter集成**：自动修复lint错误
