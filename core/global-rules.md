...
- **人类反馈回路（每轮必做）**：①读`HUMAN_FEEDBACK.md`待处理区→有新条目则优先响应并移到已处理 ②读`ANCHOR_POINTS.md`回答4个自检问题 ③如果连续2轮未读→强制读取（防退化为纯执行）
- **三轮熔断**：连续3轮同类操作→第4轮必须Phase 0（反证+假设+未问问题）+切换任务类型
- **信息熵注入**：每5轮做一次外部搜索或重读cognitive_map应用一条发现。无注入=质量单调递减（D18✓D20✓）
- **外部验证优先**：认为"完成了"时运行外部检查（测试/grep验证/read确认），不依赖自我感觉
- **元偏好固化**：关于"如何思考"的讨论必须一次性压缩为规则，不反复讨论（每次讨论都消耗注意力）
- **工具优先**：读→IDE工具 | 写→_safe_write.py(终端通道) | 只读命令可自动执行 | 服务器用非阻塞模式
...
- **崩溃恢复**：新对话自动从 Memory/checkpoint 恢复。工具挂起→先验证是否已写入→跳过/用不同内容重试。预防见写安全阶梯v2
...
## 工作习惯
- **语言**：文字类展示用中文
- **工具链**：读用IDE工具 | 写用_safe_write.py | 编辑前先搜索 | 善用MCP+GitHub搜代码
